...
pub fn solve(grid: &Grid) -> Result<Vec<Grid>, bool>{

    let mut number_of_solutions = 0;

    let mut solutions: Vec<Grid> = vec![];
    
    if try_fill(&mut grid.clone(), &mut number_of_solutions, &mut solutions){
...
fn try_fill(grid: &mut Grid, number_of_solutions: &mut usize, solutions: &mut Vec<Grid>) -> bool{
            ...
            if grid.grid[row][col] == 0{
                ...
                for current in 1..=9{ // 1-9 inclusive
                    ...
                    if check_axis(grid, row, col, current) && check_square(grid, row, col, current){
                        ...
                        let halt = try_fill(grid, number_of_solutions, solutions);
